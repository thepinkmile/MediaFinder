...
public partial class FileAnalyserService
{
    ...
    private readonly ILogger<FileAnalyserService> _logger;
    private readonly MediaFinderDbContext _dbContext;
    private readonly FFProbe _ffProbe;
    private readonly KnownVideoFileExtensions _knownVideoExtensions;
    private readonly KnownImageFileExtensions _knownImageExtensions;

    public FileAnalyserService(
        ILogger<FileAnalyserService> logger,
        MediaFinderDbContext dbContext,
        FFProbe ffProbe,
        IOptionsMonitor<KnownVideoFileExtensions> knownVideoFileExtensions,
        IOptionsMonitor<KnownImageFileExtensions> knownImageFileExtensions)
    {
        _logger = logger;
        _dbContext = dbContext;
        _ffProbe = ffProbe;
        _knownVideoExtensions = knownVideoFileExtensions.CurrentValue;
        _knownImageExtensions = knownImageFileExtensions.CurrentValue;
    }

    public async Task Run(IProgress<object> progressUpdate, CancellationToken cancellationToken = default)
    {
        // TODO: retrieve from database
        var inputs = AnalyseRequest.Create(null!, null!, null!, null!, true);
        ...
        foreach (var filepath in inputs.Files)
        {
            ...
            ++index;
            progressUpdate.Report($"Analysing file: {filepath}\nFile {index} of {inputs.Files.Count}");
            ...
            GetMetadataInfo(filepath, details, inputs.PerformDeepAnalysis, cancellationToken);
            if (cancellationToken.IsCancellationRequested)
            {
                break;
            }
            ...
            var processingTasks = new Task[] {
                GetVideoInfoAsync(filepath, details, inputs.PerformDeepAnalysis, cancellationToken),
                GetImageInfoAsync(filepath, details, inputs.PerformDeepAnalysis, cancellationToken),
                GetHashDetailsAsync(filepath, details, cancellationToken)
            };
            ...
            Task.WaitAll(processingTasks, cancellationToken);
        ...
        var result = AnalysisResponse.Create(files);
    ...
    private Task GetVideoInfoAsync(string filepath, ConcurrentDictionary<string, string> details, bool performDeepAnalysis = false, CancellationToken cancellationToken = default)
    {
        if (!performDeepAnalysis)
        {
            ...
            if (_knownVideoExtensions.Contains(details[EXTENSION_DETAIL]))
            {
    ...
    private Task GetImageInfoAsync(string filepath, ConcurrentDictionary<string, string> details, bool performDeepAnalysis = false, CancellationToken cancellationToken = default)
    {
        if (!performDeepAnalysis)
        {
            ...
            if (_knownImageExtensions.Contains(details[EXTENSION_DETAIL]))
            {
    ...
    private void GetMetadataInfo(string filepath, ConcurrentDictionary<string, string> details, bool performDeepAnalysis = false, CancellationToken cancellationToken = default)
    {
        if (!performDeepAnalysis)
        {
